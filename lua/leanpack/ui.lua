...
local function format_content()
    ...
    local content_width =
        math.max(vim.fn.strdisplaywidth("  Name  Type  Source"), max_name_width + max_type_width + max_src_width + 12)
...
local function apply_highlights(lines)
    ...
    for i, plugin in ipairs(ui_state.plugins) do
        local line_num = i + 3
        local status_hl = plugin.status == "●" and "LeanpackStatusLoaded"
            or (
                plugin.status == "◐" and "LeanpackStatusLoading"
                or (plugin.status == "✗" and "LeanpackStatusMissing" or "LeanpackStatusPending")
            )
...
local function build_plugin()
    ...
    if p and p.entry.merged_spec and p.entry.merged_spec.build then
        local spec = state.get_pack_spec(p.src)
        if spec then
            loader.load_plugin(spec, { bang = true })
        end
...
local function start_refresh_timer()
    ...
    ui_state.refresh_timer:start(
        1000,
        2000,
        vim.schedule_wrap(function()
            if
                not ui_state.buf
                or not vim.api.nvim_buf_is_valid(ui_state.buf)
                or not ui_state.win
                or not vim.api.nvim_win_is_valid(ui_state.win)
            then
                stop_refresh_timer()
                return
            end
