...
local function get_hooks()
    if not hooks_mod then
        hooks_mod = require("leanpack.hooks")
    end
    return hooks_mod
end
    ...
    local ok = create_command(prefix, function(opts)
        ...
        elseif subcommand == "build" then
            if plugin_name == "" then
                if not opts.bang then
                    vim.notify(
                        ("Use :%s build! to run build hooks for all plugins"):format(prefix),
                        vim.log.levels.WARN
                    )
                    return
                end
        ...
        elseif subcommand == "delete" then
            if plugin_name == "" then
                if not opts.bang then
                    vim.notify(
                        ("Use :%s delete! to confirm deletion of all installed plugin(s)"):format(prefix),
                        vim.log.levels.WARN
                    )
                    return
                end
        ...
        elseif subcommand == "profile" then
            ...
            if next(profile) == nil or profile._total == 0 then
                vim.notify(
                    "No profiling data available. Enable profiling by passing `profiling = { enabled = true }` to leanpack.setup()",
                    vim.log.levels.WARN
                )
                return
            end
            ...
            for _, item in ipairs(sorted) do
                table.insert(
                    lines,
                    string.format("  %s: %6.2fms", item.name .. string.rep(" ", max_name - #item.name), item.elapsed)
                )
            end
            table.insert(
                lines,
                string.format("  %s: %6.2fms", "TOTAL" .. string.rep(" ", max_name - 5), profile._total)
            )
